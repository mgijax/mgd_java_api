...
public class AlleleVariantService extends BaseService<AlleleVariantDomain> {
	...
	public SearchResults<AlleleVariantDomain> update(AlleleVariantDomain domain, User user) {
		...
		// description
		log.info("process description update");
		log.info("entity.getDescription() " + entity.getDescription());
		log.info("domain.getDescription() " + entity.getDescription());
	...
	public List<String> getHGVSByChr(String chrInfo) throws IOException, InterruptedException {
		...
		log.info("runner getStdOut: " + runner.getStdOut());		
		log.info("runner getExistCode: " + runner.getExitCode ());		

		// check exit code from RunCommand
		if (runner.getExitCode() == 0) {
			//log.info(Constants.LOG_SUCCESS_EIUTILITIES);
			results.add(runner.getStdOut());
			log.info("runner good: " + runner.getStdOut());	
			log.info("runner other: " + runner.toString());
		}
		else {
