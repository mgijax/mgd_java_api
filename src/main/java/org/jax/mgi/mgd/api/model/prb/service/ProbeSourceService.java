...
public class ProbeSourceService extends BaseService<ProbeSourceDomain> {
	...
	public SearchResults<ProbeSourceDomain> create(ProbeSourceDomain domain, User user) {
		...
		if ((!domain.getOrganismKey().equals("1") && ! domain.getOrganismKey().equals("-1")) && !domain.getStrainKey().equals("-1")) {
			log.info("settin strain key too not applicable not mouse not not resolved");
		...
		if (!domain.getCellLineKey().equals("316335") && ! domain.getAge().equals("Not Specified")) {
