...
public class VocabService extends BaseService<VocabularyDomain> {
	...
	public SearchResults<SlimVocabularyTermDomain> searchGXDVocab(String vocabKey) {	
		...
		if (vocabKey.equals("151")) {
			...
			cmd = "select _term_key as termKey, term as term, 1 as orderBy from voc_term where _vocab_key = 162 and term = 'Not Specified'" +
				"\nunion" +
				"\nselect _term_key as termKey, term as term, 2 as orderBy from voc_term where _vocab_key = 162 and term != 'Not Specified'" + 
				"\norder by orderBy, term\n";
		}			
